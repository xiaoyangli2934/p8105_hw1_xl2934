...
```
This data frame has `r nrow(df2)` rows and `r ncol(df2)` columns. The mean of `x` is `r mean(pull(df2, x))`. The median of `x` is `r median(pull(df2, x))`. The standard deviation of `x` is `r sd(pull(df2, x))`. The proportion of cases for which `x + y > 1` is `r summary(pull(df2, vec_fac))`
```{r problem 2_2}
